...
{
    Thread * threadToGet = kernel->getThreadByID(tid);
    if(threadToGet==NULL)
      return;
    else{
        Semaphore* newSemphore = new Semaphore("new semphore",0);
        this->joinSemMap_insert(tid,newSemphore);  //if it has been existed ,it doesn't matter.
        newSemphore->P(); 
...
//如果你还需要实现其他操作，请写在这里
//
unsigned int Thread::tidGenerate = 0;  //初始化tidGenerate
//end: +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
void Thread::Finish()
{
    ...
    { //begin: 如果需要，请把实验二的代码请写在这里++++++++++++++++++++++++++++++++++++++++
        // 如果不需要，请忽视
        Thread *myParent = this->parent;
        if(myParent!=NULL){
        Semaphore* newSemphore = myParent->joinSemMap_getSemByID(this->tid);
        if(newSemphore!=NULL){
            newSemphore->V();
        }
        }
