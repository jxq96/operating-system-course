    ...
    case SyscallException:
        ...
        case SC_Exec:
        {
            ...
            { //begin: 实验二的代码请写在这里++++++++++++++++++++++++++++++++++++++++
                ...
                int temp;
                int count = 0;
                char* filename = new char [128]; //128 is enough for the file name
                baseAddr=kernel->machine->ReadRegister(4);
                do{
		          kernel->machine->ReadMem(baseAddr+count,1,&temp); //read char from memory
                  filename[count]=*(char*)&temp;
                  count++;
	           }while(*(char*)&temp!='\0'&&count<128);//avoid filename too long
                if(count<128){
                    SysExec(filename);
                }
                else{
        ...
        case SC_Join:
        {
            { //begin: 实验二的代码请写在这里++++++++++++++++++++++++++++++++++++++++
                ...
                //ernel->currentThread->join(id);//join()等待你实现
        ...
        case SC_Fork:
        {
            ...
            { //begin: 实验二的代码请写在这里++++++++++++++++++++++++++++++++++++++++
                ...
                kernel->machine->WriteRegister(2,id); //写回结果
                kernel->machine->PCplusPlus();   //就是之前那三条PC的代码，现在被封装成函数了
                // 后续处理
