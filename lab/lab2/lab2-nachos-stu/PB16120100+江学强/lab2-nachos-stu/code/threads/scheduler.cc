...
{   
    ...
    (kernel->stats->totalTicks-kernel->scheduler->lastSwitchTick)/10);
    readyList->Apply(flush);
}
//end: ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
Scheduler::FindNextToRun()
{
    ...
    else
    {
        ...
        // 找到优先级最高的就绪态进程t
        if(kernel->stats->totalTicks - lastSwitchTick<MinSwitchPace||readyList->IsEmpty()){
            return NULL;
        }
        Thread* t = readyList->Front();
        if(t->getPriority()>kernel->currentThread->getPriority()){
            lastSwitchTick = kernel->stats->totalTicks;
